...
/// Trait for types that can be converted to audit data
pub trait AuditableType {
    /// Get column headers for audit output
    fn audit_headers() -> Vec<String>;
    
    /// Get audit data row as strings
    fn audit_row(&self) -> Vec<String>;
    
    /// Get audit category
    fn audit_category() -> String;
    
    /// Get audit subcategory
    ...
    /// Get custom fields beyond standard headers
