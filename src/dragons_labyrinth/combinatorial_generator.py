...
class CombinatorialGenerator:
    ...
    def generate_combinations(self, state: VariantAssetGenerationState) -> dict[str, Any]:
        ...
        combinatorial_results: dict[str, CombinatorialGeneration] = {}
        ...
        for asset_name, asset_data in assets_section.items():
            ...
            base_image_path = asset_data.get('base_image_path')
            mask_image_path = asset_data.get('mask_image_path')
            reference_image_urls = asset_data.get('reference_image_urls', [])
    ...
    def _plan_sprite_sheets(self, specs: list[VariantAssetSpec], resolution_tier: str, resolution_tiers) -> list[SpriteSheetMetadata]:
        ...
        # Plan grid layout based on 1024x1024 max sheet size
        max_sheet_px = 1024
        cell_w, cell_h = width, height
        max_cols = max(1, max_sheet_px // cell_w)
        max_rows = max(1, max_sheet_px // cell_h)
        variants_per_sheet = max_cols * max_rows
        total_sheets = math.ceil(len(specs) / variants_per_sheet) if variants_per_sheet > 0 else 0
        ...
        for sheet_idx in range(total_sheets):
            ...
            # Calculate grid size
            grid_cols = min(max_cols, max(1, len(sheet_specs)))
            grid_rows = math.ceil(len(sheet_specs) / grid_cols)
            grid_rows = min(max_rows, grid_rows)
